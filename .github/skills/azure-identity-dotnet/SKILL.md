...
description: |
  Azure Identity SDK for .NET — DefaultAzureCredential, managed identity, service principals, ASP.NET Core DI, token caching, sovereign clouds, error handling.
  Triggers: "Azure Identity", "DefaultAzureCredential", "ManagedIdentityCredential", "ClientSecretCredential", "authentication .NET", "Azure auth", "credential chain".
...
### OnBehalfOfCredential (Delegated)
...
```csharp
// Auto-configured from environment when running in AKS with WIF
...
var credential = new WorkloadIdentityCredential(
    new WorkloadIdentityCredentialOptions
    {
        TenantId = "<tenant-id>",
        ClientId = "<client-id>",
        TokenFilePath = "/var/run/secrets/tokens/azure-identity"
    });
```
...
- **Mirror of:** `azure-identity-py` (same concepts, different language)
- **→ `entra-app-registration`** — Service principal registration for ClientSecretCredential / ClientCertificateCredential
- **→ `entra-managed-identity`** — Azure MI provisioning for ManagedIdentityCredential
- **→ `entra-workload-identity-federation`** — WIF setup for WorkloadIdentityCredential
- **→ `entra-agent-id-runtime`** — OBO flow uses OnBehalfOfCredential pattern
- **→ `azure-rbac`** — RBAC role assignment for authenticated identities
- **→ `entra-secret-certificate-lifecycle`** — Credential rotation for secrets and certificates
- **See also:** `entra-msal-deep-dive` (lower-level MSAL.NET patterns)
